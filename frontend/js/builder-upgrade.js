(function(){
  ...
  function exportXmlSkeleton(){
    ...
    const rows = [
      ...
      '  </blocks>',
      '  <warning>This is an ApexBot XML export skeleton for review/import workflows. Test in demo before live use.</warning>',
      '</apexbot_strategy>'
    ];
  ...
  function blockCount(type){ return $$('.visual-node.' + type).length; }
  ...
  function updateInspector(){
    ...
    if (!hasRisk) warnings.push('Add Risk Management before any account connection.');
  ...
  function buildCommandCenter(){
    ...
    div.innerHTML = `
      <div class="builder-command-card"><span class="builder-kicker">Bot builder workflow</span><h3>Build like DBot, explain like ApexBot</h3><p>Pick a ready template, select a market, confirm contract type, add entry logic, lock risk controls, then run demo/backtest before saving.</p><div class="builder-workflow"><div class="workflow-step" data-step="template"><b>Template</b><span>Start with a known structure</span></div><div class="workflow-step" data-step="market"><b>Market</b><span>Volatility, Boom/Crash, Forex, Crypto</span></div><div class="workflow-step" data-step="contract"><b>Contract</b><span>Rise/Fall, Digits, Touch, Multiplier</span></div><div class="workflow-step" data-step="conditions"><b>Signal</b><span>RSI, EMA, digit, AI filter</span></div><div class="workflow-step" data-step="risk"><b>Risk</b><span>Stops, max trades, daily loss</span></div><div class="workflow-step" data-step="test"><b>Test</b><span>Demo run and backtest</span></div></div></div>
      <div class="builder-command-card"><span class="builder-kicker">One-click templates</span><h3>Load popular Deriv bot styles</h3><p>These buttons use your existing template engine, then you can edit every block on the canvas.</p><div class="quick-template-grid"><button class="quick-template hot" data-load-template="over"><b>Over / Under</b><span>Digits learner with prediction</span></button><button class="quick-template hot" data-load-template="even"><b>Even / Odd</b><span>Digit pattern starter</span></button><button class="quick-template" data-load-template="ema"><b>EMA Rise/Fall</b><span>Trend-following structure</span></button><button class="quick-template" data-load-template="rsi"><b>RSI Reversal</b><span>Mean-reversion structure</span></button><button class="quick-template" data-load-template="boom"><b>Boom Guard</b><span>Volatility filter + risk</span></button><button class="quick-template" data-load-template="ai"><b>AI Approval</b><span>Scanner confirmation gate</span></button></div></div>
      <div class="builder-command-card"><span class="builder-kicker">Readiness meter</span><h3>Know if the bot is test-ready</h3><p>This checks structure and risk controls. It does not predict profit.</p><div class="builder-progress"><span id="builder-readiness-bar"></span></div><div class="builder-score-grid"><div class="builder-score"><small>Readiness</small><strong id="builder-readiness">0%</strong></div><div class="builder-score"><small>Risk</small><strong id="builder-risk">—</strong></div><div class="builder-score"><small>Market</small><strong id="builder-market">—</strong></div><div class="builder-score"><small>Contract</small><strong id="builder-contract">—</strong></div></div></div>`;
    topbar.after(div);
  ...
  function buildInspector(){
    ...
    div.innerHTML = `<h4>Strategy inspector</h4><div id="builder-live-summary" class="builder-summary-list"></div><div id="builder-live-warnings" class="builder-warnings"></div><div class="builder-upgrade-actions"><button id="builder-validate-now" class="btn ghost" type="button">Refresh checks</button><button id="builder-export-xml" class="btn ghost" type="button">Export XML skeleton</button></div><div class="builder-export-note">JSON export remains the main ApexBot format. XML skeleton export helps users inspect/share strategy structure without claiming guaranteed DBot compatibility.</div>`;
  ...
  function setSelectValue(id, value){
    ...
    const option = Array.from(select.options || []).find((o)=>o.value === value || o.textContent.includes(value));
  ...
  function applyUrlPreset(){
    ...
    if (template) {
      ...
      addUpgradeLog(`Template preset loaded from link: ${template}.`);
    }
    setTimeout(()=>{
      ...
      setTimeout(()=>{
        const okSymbol = setSelectValue('p-symbol', symbol);
        const okContract = setSelectValue('p-contract', contract);
        if (okSymbol || okContract) addUpgradeLog('Market/contract preset applied from template link. Review risk controls before demo testing.');
      }, 500);
    }, 700);
  }
